...
do
    ...
    $CURL -H "Content-Type: application/json" -XPUT $ELASTICSEARCH/$KIBANA_INDEX/search/$name \
        -d @$file || exit 1
    echo
done
...
do
    ...
    $CURL -H "Content-Type: application/json" -XPUT $ELASTICSEARCH/$KIBANA_INDEX/visualization/$name \
        -d @$file || exit 1
    echo
done
...
do
    ...
    $CURL -H "Content-Type: application/json" -XPUT $ELASTICSEARCH/$KIBANA_INDEX/dashboard/$name \
        -d @$file || exit 1
    echo
done
...
do
    ...
    $CURL -H "Content-Type: application/json" -XPUT $ELASTICSEARCH/$KIBANA_INDEX/index-pattern/$name \
        -d @$file || exit 1
    echo
done
